...
/**
 * Mapping of pathway IDs to their proper display names.
 * Handles casing that naive capitalize cannot (e.g. DevSecOps).
 */
...
function getPathwayDisplayName(pathwayId: string): string {
  return (
    PATHWAY_DISPLAY_NAMES[pathwayId] ||
    pathwayId
      .split('-')
 ...
 *
 * Route: /dashboard/certifications/detail?pathway={pathway_id}
 *
 * Uses useSearchParams() to read the pathway query parameter for
 * static export + CloudFront compatibility (no dynamic route segments).
 *
 * Requirements: 12.3, 12.4, 12.6
 */
function PathwayDetailContent() {
  ...
  useEffect(() => {
    if (!pathwayId) {
      setIsLoading(false);
      setError('No pathway specified. Please select a pathway from the certifications page.');
      return;
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (candidateRes.error) {
          // If 404, the user hasn't enrolled — show "not enrolled" state (not an error)
          if (candidateRes.error.includes('404') || candidateRes.error.includes('No candidate record')) {
            // Leave candidate as null — the "not enrolled" UI will render
            setIsLoading(false);
            return;
          }
  ...
  async function handleSubmitCapstone(e: React.FormEvent) {
    ...
    if (!pathwayId || !submissionUrl.trim()) return;
  ...
  /**
   * Download reviewer feedback as a text file
   */
  function downloadFeedback(notes: string, revisionNumber: number, pathway: string) {
    const blob = new Blob([notes], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `capstone-feedback-${pathway}-rev${revisionNumber}.txt`;
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
            ...
            {error && !isLoading && (
              <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                ...
                {/* Not enrolled state — user hasn't started this pathway */}
                {!isLoading && !error && !candidate && pathwayId && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                      You haven't started this certification pathway yet. Complete the required coursework and submit your capstone project to obtain this certification.
                    </p>
                    <Link
                      href="/dashboard/certifications"
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-500 dark:bg-amber-400 text-gray-900 font-semibold text-sm hover:bg-amber-600 dark:hover:bg-amber-500 transition-colors"
                    >
                      ← Back to Certifications
                    </Link>
                  </div>
            ...
            {!isLoading && !error && candidate && pathwayId && (
              <div className="space-y-6">
                ...
                {/* Celebration state — when AWARDED */}
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="text-center py-6">
                      ...
                      <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                        You&apos;ve demonstrated your expertise in {getPathwayDisplayName(pathwayId)}. Your credential is active and publicly verifiable.
                      </p>
                    </div>

                    {/* Certificate preview */}
                    <div className="max-w-2xl mx-auto">
                      <CertificatePreview pathwayId={pathwayId} />
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-900/10 dark:to-yellow-900/10 border border-amber-200 dark:border-amber-800 rounded-2xl p-6 sm:p-8">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                        <div>
                          ...
                          <p className="text-lg sm:text-xl font-mono font-bold text-gray-900 dark:text-gray-100">
                            {credential.credential_id}
                          </p>
                        </div>
                        <span className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 border border-green-200 dark:border-green-800">
                          ✓ Active
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Pathway</p>
                          <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{getPathwayDisplayName(pathwayId)}</p>
                        </div>
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Issued</p>
                          <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                            {new Date(credential.issued_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Expires</p>
                          <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                            {new Date(credential.expires_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                          </p>
                        </div>
                      ...
                      {/* Action buttons */}
                      <div className="flex flex-wrap gap-3 pt-4 border-t border-amber-200 dark:border-amber-700">
                        <CertificateDownload
                ...
                {/* Non-awarded state — show review status, submission form, etc. */}
                {candidate.candidate_status !== 'AWARDED' && (
                  <>
                ...
                {/* Review history section — always visible */}
                {reviews.length > 0 && (
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {session.reviewer_notes && (
                                <div className="mt-2">
                                  ...
                                  <button
                                    onClick={() => downloadFeedback(session.reviewer_notes!, session.revision_number, pathwayId!)}
